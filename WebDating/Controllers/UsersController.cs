...
namespace WebDating.Controllers
{
    ...
    public class UsersController : BaseController
    {
        ...
        public async Task<ActionResult<MemberDto>> GetMember(string username)
        {
            ...
            user.DatingProfile = new();
            user.DatingProfile.DatingObject = dating.DatingObject.GetDisplayName();
            user.DatingProfile.Height = dating.Height.GetDisplayName();
            user.DatingProfile.WhereToDate = dating.WhereToDate.GetDisplayName();

            user.DatingProfile.UserInterests = [];
            foreach (var item in dating.UserInterests)
            {
                UserInterestDto interest = new()
                {
                    InterestName = dating.UserInterests.FirstOrDefault(x => x.Id == item.Id).InterestName.GetDisplayName(),
                    Id = item.Id,
                    DatingProfileId = item.DatingProfileId
                };
                user.DatingProfile.UserInterests.Add(interest);
            }
